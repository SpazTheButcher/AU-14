...
public sealed partial class HydroTrayEffectSystem : EntitySystem
{
    ...
    private void Hepatopeutic(ref HydroTickEvent<Hepatopeutic> args)
        => Mutate(args.Potency, args.Args, MutationSystem.TargetedPlantMutation.Negative);

    private void Nephropeutic(ref HydroTickEvent<Nephropeutic> args)
        => Mutate(args.Potency, args.Args, MutationSystem.TargetedPlantMutation.Tolerances);

    private void Pneumopeutic(ref HydroTickEvent<Pneumopeutic> args)
        => Mutate(args.Potency, args.Args, MutationSystem.TargetedPlantMutation.GrowthAndHealth);

    private void Oculopeutic(ref HydroTickEvent<Oculopeutic> args)
        => Mutate(args.Potency, args.Args, MutationSystem.TargetedPlantMutation.Potency);

    private void Neuropeutic(ref HydroTickEvent<Neuropeutic> args)
        => Mutate(args.Potency, args.Args, MutationSystem.TargetedPlantMutation.Species);
    ...
    private void Mutate(FixedPoint2 potency, EntityEffectHydroArgs args, MutationSystem.TargetedPlantMutation field)
    {
        if (!CanMetabolizePlant(args.TargetEntity, out var plant, mustHaveMutableSeed: true) || plant.Seed == null)
            return;
        _plantHolder.EnsureUniqueSeed(args.TargetEntity, plant);
        var seed = plant.Seed;
        if (seed == null)
            return;
        _mutation.MutateSeedField((args.TargetEntity, plant), ref seed, field, HydroStrength(potency, args));
        plant.Seed = seed;
    }
