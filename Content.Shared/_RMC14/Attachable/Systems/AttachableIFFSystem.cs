...
public sealed partial class AttachableIFFSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<GunIFFComponent, AttemptShootEvent>(OnGunIFFAttemptShoot);
    }
    ...
    private void CheckPreventFriendlyFire(EntityUid weapon, ref AttemptShootEvent args)
    {
        if (args.Cancelled ||
        ...
        try
        {
            ...
            var shotDirection = GetPredictedShotDirection((weapon, gun), initialDirection);
            if (shotDirection == Vector2.Zero)
                return;

            var maxDistance = GetIffCheckDistance((weapon, gun));
            if (maxDistance <= 0)
                return;

            var shotEnd = from.Position + shotDirection * maxDistance;
            var shotRadius = GetIffCheckRadius((weapon, gun));
